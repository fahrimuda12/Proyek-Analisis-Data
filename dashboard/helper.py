class Analyzer:
   ...
   def create_revenue_top_product_df(self):
      revenue_top_product_df = self.df.groupby("product_category_name_english")["payment_value"].sum().reset_index()
      revenue_top_product_df = revenue_top_product_df.sort_values(by='payment_value', ascending=False)

      return revenue_top_product_df

   def create_revenue_top_yearly_df(self):
      # Filter data untuk kategori produk yang diinginkan
      annual_revenue = self.df.groupby([self.df['order_approved_at'].dt.year, 'product_category_name_english'])['payment_value'].sum().reset_index()
      annual_revenue.rename(columns={
         'order_approved_at': 'year'
      }, inplace=True)
      annual_revenue = annual_revenue.sort_values(by=['year', 'payment_value'], ascending=[True, False])
